...
- 搜索、标签、合规状态、未推送条件和排序均同步到 URL，刷新后保留，也可直接分享同一筛选视图。
- 概览卡可直接进入“满足推送阈值”“待复核”“尚未推送”列表；标签支持二次搜索和展开全部。
- 列表请求可取消旧查询，避免连续筛选时旧结果覆盖新结果；滚动临近底部自动加载下一页。
- 详情支持上一篇/下一篇、图片放大与键盘方向键导航，原笔记链接可一键复制。
- 单图使用三态快捷按钮；整篇可批量标为符合或不符合，批量操作需确认，单图、批量和推送状态均提供撤销入口。
- 图片卡同时显示 AI 判断、判断说明与人工复核来源；人工备注可独立保存，切换状态和批量操作不会清空已有备注。
- 人工备注输入后 700ms 自动保存并就地显示状态；切换笔记、关闭详情或批量复核前会刷新待保存内容，保存失败时保留输入并明确提示。
- 移动端筛选以抽屉呈现，避免占用首屏；复核图片改为单列大图，降低误判和误触。
